...
public class HexDump
{
	...
	public String getHexString( /*FileInputStream fis*/ ) {
		String s = "Hier entsteht ein HexDump, Fertigstellung in ca. 1 Stunde";
		char[] h=new char[2];
		String o = "";
		
		for( int i=0; i < s.length ; i++ ) {
			h = hexByte( (int)s.substring( i, 1 ), 2 );
			o += h[0]+h[1];
		}
		//getHexByte
	...
	public static void main( String args[] ) {
		...
		//~ try {
			//~ System.out.println( new HexDump( ( args.length > 0 && args[0] != "" ) ? new FileInputStream( args[0] ) : "" ).getHexString() );
		//~ } catch( Exception e ) {
			//~ System.out.println( e );
		//~ }
	}
