...
\begin{tikzpicture}[node distance = 1.5cm, auto, thick, nodes = {align = center},
	...
	\node [block, right of=start, node distance=4.5cm] (readx) {x};
	\path [line] (start) -- (readx);
	\node [block, right of=readx, node distance=4.5cm] (reade) {epsilon};
	\path [line] (readx) -- (reade);
	\node [block, below of=reade, node distance=1.5cm] (initsum2) {sum2=1.0};
	\path [line] (reade) -- (initsum2);
	\node [block, left of=initsum2, node distance=4.5cm] (initsum1) {sum1=0.0};
	\path [line] (initsum2) -- (initsum1);
	\node [block, left of=initsum1, node distance=4.5cm] (initn) {n=0};
	\path [line] (initsum1) -- (initn);
	\node [decision, below of=initn, node distance=3cm] (decide) { (sum2 - *sum1) gt epsilon };
		\path [line] (initn) -- (decide);
    		\node [block, right of=decide, yshift=-1cm, node distance=4.5cm] (setsum1) {sum1 = sum2};
		\path [line] (decide) -- node [near start, below] {yes} (setsum1);
    		\node [block, right of=setsum1, yshift=1cm, node distance=4.5cm] (setsum2) {sum2 += pow(x, n)/fac(n)};
		\path [line] (setsum1) -- (setsum2);
    		\node [block, above of=setsum1, yshift=0.8cm, node distance=1.5cm] (setn) {n++};
